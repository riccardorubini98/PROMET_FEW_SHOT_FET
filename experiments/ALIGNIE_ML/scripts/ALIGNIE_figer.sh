...
DATACONFIG='configs/data/figer_s'$SUBSET'.yaml'

CUDA_VISIBLE_DEVICES=1 $CMD trainers/trainer_alignie.py fit \
--seed_everything=$SEED \
--data configs/data/common_alignie_figer.yaml \
--data $DATACONFIG \
...
--model configs/model/classifier_alignie_figer.yaml \
--logger configs/logger.yaml \
--logger.project='FIGER' \
--logger.name='ALIGNIE_BEST' \
